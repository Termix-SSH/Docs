...
Termix covers a lot of ground. This page is a map of what's documented, grouped by area.

## Authentication

Local accounts, SSO providers (OIDC, LDAP, GitHub, Google), two factor auth, RBAC sharing, and SSH certificate login. See [Authentication](./authentication/overview).

## Networking

Tunnels, host metrics, Docker management, SSL, Tailscale, port knocking, and Wake on LAN. See the Networking pages in the sidebar for each one.
...
A customizable home screen built from cards, including a network topology graph. See [Dashboard](./dashboard/overview).

## API

API keys for scripting against the Termix API. See [API Keys](./api/api-keys).
